...
%m = input ('please enter the number of time nodes,     m = ');
m=n;
T=1;
%%%%%%%%%%%%%%%%%%%%%%%    Omega(alpha)   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
b=-0.1*ones((n+1)*(m+1),1);
vbar = observation_value(n,m,t,x,w);
u = control_value(n,m,x,t);
%q = source_value(n,m,x,t);
q = -u;
lb=[];
ub=[];
%%%%%%%%%%%%%   quadratic form   %%%%%%%%%%%%%%%%%%%%%%
...
eJ=J_exact-J_numerical;
 ...
 eu = u-y((((m+1)*(n+1))+1):(2*((m+1)*(n+1))));
  error = [norm(ev,2)/((n+1)), norm(eu,2)/((n+1)),norm(eJ,2)/((n+1))]'
...
set(gca, 'Position', get(gca, 'OuterPosition') - ...
    get(gca, 'TightInset') * [-1 0 1 0; 0 -1 0 1; 0 0 1 0; 0 0 0 1]);
pbaspect([6,6,6])
xlabel t, ylabel x, zlabel v_{ex}(x,t)-v_{50}(x,t)
...
set(gca, 'Position', get(gca, 'OuterPosition') - ...
    get(gca, 'TightInset') * [-1 0 1 0; 0 -1 0 1; 0 0 1 0; 0 0 0 1]);
pbaspect([6,6,6])
xlabel t, ylabel x, zlabel u_{ex}(x,t)-u_{50}(x,t)
